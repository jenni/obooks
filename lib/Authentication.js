...
class GoogleAuthentication {
    ...
    constructor(email, password) {
        ...
        this.selectors = {
            ...
            google: {
                ...
                twoFA: ['input[id="authzenToken"]', 'input[id="azAction"]']
            }
        }
    }
    ...
    async login(page) {
        try {
            ...
            const opts = {
                ...
                twoFA:  this.selectors.google.twoFA,
            ...
            await this.checkForPromptIssues(page)
    ...
    async checkForPromptIssues(page) {
        if ((await page.content()).match(/The sign-in prompt couldn\'t reach/gi)) {
            log('Cannot log in to Gmail through Obooks. '
                + 'There was an error sending prompt to your phone, please try again later.')
            process.exit(1)
        }
        if ((await page.content()).match(/Too many Google prompts/gi)) {
            log('Cannot log in to Gmail through Obooks. ' 
                + 'Too many Google prompts already sent, please try again later.')
            process.exit(1)
        }
    }
    ...
    async getLoginUrl() {
        ...
        const loginUrl = body.redirect_uri

        if (body.is_corporate === true) {
            log(`redirecting to corporate login`)
            try {
                const res = await req('GET', loginUrl, null, null)
...
const getElement = async (page, arr, timeout = 5000) => {
    const promisedElements = arr.map(el => page.waitForSelector(el, { visible: true, timeout }))

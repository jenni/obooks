...
class OBook {
    ...
    async downloadChapterImages(chapter, bookFolderPath) {
        const imagePaths = chapter.images
        const assetBasetUrl = chapter.asset_base_url
        if (!imagePaths.length || !isUrl(assetBasetUrl)) return

        return this.parallelDownloadImages(
            imagePaths,
            assetBasetUrl,
        ...
        try {
            const requests = imagePaths.map(imagePath => {
                return async function () {
                    ...
                    return downloadIMG({ url: oReillyImageUrl, dest })
                }
            })
            return async.parallel(requests, (err, result) => result)
        } catch (e) {
            console.error(e)
            process.exit(1)
        }

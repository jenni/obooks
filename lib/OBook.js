...
class OBook {
    ...
    constructor(cookie, bookid) {
        ...
        this.imageDestinationPath = IMG_DEST_PATH + this.bookid + '/'
    }
    ...
    async create() {
        ...
        for (const chapterUrl of chapterUrls) {
            ...
            const finalContent = this.replaceImagePaths(content)
        ...
        return new Epub(epubOpts, `${EBOOK_DEST_PATH}/${filename}.epub`)
    }
    ...
    replaceImagePaths(content) {
        ...
        const localImagesPathDest = `src="${this.imageDestinationPath}`;
        return $.html().replace(/src="/g, localImagesPathDest)
    }
    ...
    async downloadChapterImages(chapter) {
        ...
        for (const image of images) {
            const bookImagesPath = this.imageDestinationPath
            const imageFilename = image.split('/').pop()
            const dest = this.imageDestinationPath + imageFilename

            if (!existsSync(bookImagesPath)) mkdirSync(bookImagesPath, { recursive: true })

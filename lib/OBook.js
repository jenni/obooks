...
class OBook {
    ...
    async create() {
        ...
        let chapter, chapterResult = [], chapterUrl = meta.chapters
        do {
            chapter = await client.fetch(chapterUrl, this.cookie)
            chapterResult = [...chapterResult, ...chapter.results]
            chapterUrl = chapter.next

            if (!!chapterUrl) log(`fetching next chapter ...`)
        } while (!!chapterUrl)
        
        const cover = await this.fetchMaybeCoverImageUrl(chapterResult)
